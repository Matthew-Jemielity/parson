...
#   warranty.
#
#   Tiny mods by Mateusz Jemielity, m.jemielity@is-wireless.com, 2016/09/06
...
ifeq ($(VALGRIND_ENABLED),yes)
	-$(A''M_V_at)$(foreach tool,$(valgrind_enabled_tools), \
		$(MAKE) $(AM_MAKEFLAGS) check-valgrind-$(tool); \
	)
else
...
#
# Removed the equals sign after var name.
# With it, make 3.81 got borked and did not generate the per-tool targets.
# Everywhere I looked at, the equals sign should have worked. Do not know yet
# what is going on - Ubuntu thing? Removing it for now.
# Also changed target name from check-TESTS to just check.
#     - Mateusz
